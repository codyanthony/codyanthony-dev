...
`trailingSlash: "ignore"` in `astro.config.mjs` — both `/about` and `/about/` resolve. It is **not** `"always"`: the R2 asset proxy serves extensionful URLs like `/assets/blog/og.png`, which must work *without* a trailing slash (OG `<meta>` images, `<img src>`). Existing trailing-slash page URLs and the `_redirects` rules still resolve; canonical `<link>` tags in `BaseLayout` handle SEO dedup.
...
- `pnpm astro check` — TypeScript / Astro compile errors and warnings. Must be 0/0/0 before committing.
...
`eslint.config.mjs` lints only `.mjs` files. Astro/MDX/JSX content files are not in scope.
